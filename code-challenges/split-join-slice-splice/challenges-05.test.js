...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach((item) => { 
    const array = item.split(' ').splice(2);
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach((step) => {
    ...
    result.push(step.slice(0, index));
  })
  return result;
};
...
const extractVowels = (str) => {
  ...
  array.push(str.replace(regex,''));
  const sortedVowels = str.match(regex).sort((a,b) => a > b);
  array.push(sortedVowels.join(''));
  return array;
};
