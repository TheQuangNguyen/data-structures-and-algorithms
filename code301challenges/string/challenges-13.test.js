...
const onlyOddChars = (str) => {
  return str.split('').reduce((accum,letter,index) => {
    if(index%2 !== 0) { 
      return accum + letter;
    } else { 
      return accum;
...
const allHappy = (arr) => {
  return arr.filter(string => string.includes(':)')).length === arr.length;
};
...
const unenrollBrook = (arr) => {
  return arr.map(array => {
    return array.filter(name => !name.includes('Brook'));
  })
};
...
const sortByDay = (arr) => {
  const eventArray = daysOfWeek.map(day => []);
  arr.forEach(event => {
    daysOfWeek.forEach((day, index) => {
      if (event.includes(day)) { 
        eventArray[index].push(event);
      }
    })
  });
  return eventArray;
