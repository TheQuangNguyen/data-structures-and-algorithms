...
const count = (target, input) => {
  return input.reduce((accum, array) => { 
    return accum + array.filter(num => num === target).length;
  }, 0)
};
...
const totalSum = (input) => {
  return input.reduce((accum, array) => { 
    return accum + array.reduce((acc, num) => { 
      return acc + num;
    }, 0);
  }, 0);
};
...
let findShortest = (data) => {
  return data.reduce((accum, object) => {
    if (accum.name.length < object.name.length) { 
      return accum;
    } else { 
      return object;
    }
