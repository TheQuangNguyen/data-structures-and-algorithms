...
const validateEmail = (email) => {
  const regex = /^[a-z]+.?[a-z]+@[a-z]+.(com|net|org)$/gi;
...
const validatePhoneNumber = (phoneNumber) => {
  const regex = /^(\([0-9]{3}\))?([0-9]{3})?[\s-]?\s?[0-9]{3}[-\s]?[0-9]{4}$/g; 
...
const findTagNames = elements => {
  ...
  const result = elements.reduce((accum,string) => {
    return accum + string.match(regex).join(',') + ',';
  },'').split(',');
