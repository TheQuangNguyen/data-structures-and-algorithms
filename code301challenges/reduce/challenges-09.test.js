...
const countNumberOfElements = (arr) => {
  return arr.reduce((accumulator, element) => accumulator + 1);
};
...
const returnNames = (arr) => {
  return arr.reduce((nameArray, currentObject) => {
    nameArray.push(currentObject.name);
    return nameArray;
  }, []);
};
...
const reversedString = (str) => {
  const arr = str.split('');
  return arr.reduce((word, letter) => letter + word, '');
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((numChildren, element) => { 
    if (element.children) { 
      return numChildren + element.children.length;
    } else {
      return numChildren;
    }
  }, 0)
...
const calculateAverage = (arr) => {
  const obj = arr.reduce((average, num) => {
    average.count += 1;
    average.sum += num;
    return average;
  }, {count:0, sum:0});
...
const extractStat = (statName, arr) => {
  const result = arr.reduce((accum, object) => { 
...
const extractChildren = (arr) => {
  const filter = arr.filter(object => {
    const regex = /[a]+/gi;
    return regex.test(object.name)
  })
  
  console.log(filter);
  const reduce = filter.reduce((accum, element) => { 
    if(element.children) { 
      console.log(accum + element.children.join(',') + ',')
      return accum + element.children.join(',') + ',';
    } else { 
      return accum;
    }
  }, '').split(',');
